...
int ProducerConsumerTest::start(int argc, char* argv[]) {
  ...
  this->consumers[this->consumerCount - 1] =
    new AssemblerTest(losingPackages, this->consumerCount);
  assert(this->consumers[this->consumerCount - 1]);
  this->consumers[this->consumerCount - 1]->createOwnQueue();
  static_cast<AssemblerTest*>(this->consumers[this->consumerCount - 1])
