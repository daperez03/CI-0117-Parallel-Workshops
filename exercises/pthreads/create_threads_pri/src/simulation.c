...
int join_threads(size_t count, privated_data_t* privated_data);
...
privated_data_t* create_threads
  (size_t count, void*(*subroutine)(void*), void* data) {
  privated_data_t* privated_data = (privated_data_t*)
    calloc(count, sizeof(privated_data_t));
  if (privated_data) {
    for (size_t index = 0; index < count; ++index) {
      privated_data[index].data_shared = (simulation_t*)data;
      privated_data[index].thread_number = index;
      privated_data[index].threads_number = count;
      privated_data[index].my_id = (pthread_t*) calloc(1, sizeof(pthread_t));
      if (pthread_create(privated_data[index].my_id, /*attr*/ NULL, subroutine
        , (void*)&privated_data[index]) == EXIT_SUCCESS) {
      } else {
        fprintf(stderr, "error: could not create thread %zu\n", index);
        join_threads(index, privated_data);
        return NULL;
      }
    }
  }
  return privated_data;
}
...
int join_threads(size_t count, privated_data_t* privated_data) {
  ...
  for (size_t index = 0; index < count; ++index) {
    ...
    error += pthread_join(*privated_data[index].my_id , /*value_ptr*/ NULL);
    free(privated_data[index].my_id);
  }
  free(privated_data);
  return error;
}
...
int create_consumers_producers(simulation_t* simulation) {
  ...
  privated_data_t* producers = create_threads
    (simulation->producer_count, produce, simulation);
  privated_data_t* consumers = create_threads
    (simulation->consumer_count, consume, simulation);
