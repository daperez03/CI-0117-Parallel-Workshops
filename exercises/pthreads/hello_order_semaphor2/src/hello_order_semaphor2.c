...
int create_threads(shared_data_t* share_data) {
  ...
  if (threads && private_data) {
    for (uint64_t thread_number = 0; thread_number < share_data->thread_count
      ; ++thread_number) {
      //  inicializamos el vector de semaforos
      error = sem_init(&share_data->can_greet[thread_number], 0, 0);
    ...
    //  Se aumenta el semafor en posicion 0 a 1
    sem_post(&share_data->can_greet[0]);
    for (uint64_t thread_number = 0; thread_number < share_data->thread_count
      ; ++thread_number) {
