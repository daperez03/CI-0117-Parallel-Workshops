...
void test_arrays(array_t* array1, array_t* array2) {
  ...
  size_t thread_count = 10 + rand() % 20; // NOLINT
  pthread_t* threads = (pthread_t*) malloc(thread_count * sizeof(pthread_t));
  for (size_t current = 0; current < thread_count; ++current)
    pthread_create(threads + current, NULL
      , test_array, current % 2 ? array2 : array1);
  for ( size_t current = 0; current < thread_count; ++current)
...
void* test_array(void* data) {
  ...
  for (size_t index = 0, count = 1000
    + rand() % 10000; index < count; ++index) {  // NOLINT
    size_t num = rand() % 100;  // NOLINT
    if ( num >= 10 ) {
      array_append( array, (void*)(num) );
    } else {
      num += rand() % 90;  // NOLINT
      if ( array_find_first(array, (void*)num, 0) != array_not_found )
        array_remove_first(array, (void*)num, 0);  // NOLINT
    }
  }
