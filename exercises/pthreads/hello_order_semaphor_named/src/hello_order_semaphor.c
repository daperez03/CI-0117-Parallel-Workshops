...
int main(int argc, char* argv[]) {
  ...
  if (share_data) {
    ...
    if (share_data->can_greet) {
      ...
      for (uint64_t thread_number = 0; thread_number < share_data->thread_count; ++thread_number) {
        char _name[256];
        snprintf(_name, (size_t)256, (const char*)"sem%zu", thread_number);
        sem_unlink((const char*)_name);
      }
  ...
  if (threads && private_data) {
    for (uint64_t thread_number = 0; thread_number < share_data->thread_count
      ...
      share_data->can_greet[thread_number] =
        sem_open((const char*) _name, O_CREAT | O_EXCL, 0644, !thread_number);
      if (error == EXIT_SUCCESS && errno == EXIT_SUCCESS) {
