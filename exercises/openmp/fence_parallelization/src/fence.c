...
coordinate_t find_maximum_local_perimeter(const size_t top_left_row
  , const size_t top_left_column, const fence_data_t* fence_data);
...
void find_maximum_perimeter(fence_data_t* fence_data) {
  ...
  #pragma omp parallel num_threads(thread_count) \
        default(none) shared(fence_data)
  {
    for ( size_t top_left_row = 0; top_left_row < fence_data->rows - 1
      ; ++top_left_row ) {
      #pragma omp for schedule(runtime)
      // Repeat top left column for each column of terrain except last one
      for ( size_t top_left_column = 0
        ; top_left_column < fence_data->columns - 1; ++top_left_column ) {
        ...
        coordinate_t local_bottom_right = find_maximum_local_perimeter
          (top_left_row, top_left_column, fence_data);
        if ( local_bottom_right.row > 0 || local_bottom_right.column > 0 ) {
          ...
          #pragma omp critical
          {
            ...
            if ( local_perimeter > fence_data->maximum_perimeter ) {
              ...
              // Assign maximum coordinates to the local
...
// Find maximum perimeter that can be formed
// from (top left row, top left column):
coordinate_t find_maximum_local_perimeter(const size_t top_left_row
  , const size_t top_left_column, const fence_data_t* fence_data) {
    ...
    for ( size_t bottom_right_column = top_left_column + 1
      ...
      if ( can_form_rectangle(top_left_row, top_left_column, bottom_right_row
        ...
        size_t perimeter = calculate_perimeter((coordinate_t)
          {top_left_row, top_left_column}, (coordinate_t)
        ...
        if ( perimeter > local_maximum_perimeter ) {
          ...
          local_maximum_coordinates = (coordinate_t)
            {bottom_right_row, bottom_right_column};
        }
      }
    }
...
bool can_form_rectangle(const size_t top_left_row, const size_t top_left_column
  ...
  for ( size_t column = top_left_column
    ...
    if ( fence_data->terrain[top_left_row][column] == 'x'
      || fence_data->terrain[bottom_right_row][column] == 'x' ) {
      // Return false
      return false;
    }
  }
  ...
  for ( size_t row = top_left_row + 1; row <= bottom_right_row - 1; ++row ) {
    ...
    if ( fence_data->terrain[row][top_left_column] == 'x'
      ||  fence_data->terrain[row][bottom_right_column] == 'x' ) {
      // Return false
      return false;
    }
  }
...
size_t calculate_perimeter(const coordinate_t top_left
  , const coordinate_t bottom_right) {
  /* Return 2 * (bottom right column - top left column) + 2 *
  (bottom right row - top left row)*/
...
void print_maximum_perimeter(const fence_data_t* fence_data) {
  printf("%zu (%zu,%zu)-(%zu,%zu)\n", fence_data->maximum_perimeter
    , fence_data->maximum_top_left.row + 1
      , fence_data->maximum_top_left.column + 1
