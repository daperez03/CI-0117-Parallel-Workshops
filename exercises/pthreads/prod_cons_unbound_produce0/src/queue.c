...
/// @brief Funcion para saver si la cola no es vacia
/// @param queue cola con datos
/// @return vacia un con contenido
...
bool queue_is_empty(queue_t* queue) {
  ...
  pthread_mutex_lock(&queue->can_access_queue);
    bool result = queue->head == NULL;  //  reivisa si la lista es vacia
  pthread_mutex_unlock(&queue->can_access_queue);
  return result;
}

bool queue_is_empty_unsafe(queue_t* queue) {  //  is_empty privado y unsafe
  assert(queue);
