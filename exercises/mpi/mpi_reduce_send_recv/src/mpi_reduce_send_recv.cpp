...
void generate_lucky_statistics(int process_number, int process_count) {
  ...
  if (reduce(my_lucky_number, all_min, MPI_MIN) != MPI_SUCCESS) {
    fail("error: could not reduce min");
  }
  // std::cout << std::endl << std::endl;
  if (reduce(my_lucky_number, all_max, MPI_MAX) != MPI_SUCCESS) {
    fail("error: could not reduce max");
  }
  // std::cout << std::endl << std::endl;
  if (reduce(my_lucky_number, all_sum, MPI_SUM) != MPI_SUCCESS) {
